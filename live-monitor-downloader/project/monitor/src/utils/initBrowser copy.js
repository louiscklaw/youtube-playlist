...
async function testStealthing(page) {
  try {
    ...
    assert(
      webdriver_result == "missing (passed)",
      "webdriver should be missing !"
    );
    assert(chrome_result == "present (passed)", "should present !");

    await page.goto("http://bait:8080/navigator_webdriver_test.html", {
      waitUntil: "load",
    });
    ...
    assert(
      webdriver_result == "undefined",
      "navigator.Webdriver not equal to undefined"
    );
  } catch (error) {
